    ...
    public class ARCoreSessionRecorder : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (m_Session.subsystem is ARCoreSessionSubsystem subsystem)
            {
                ...
                {
                    if (playbackStatus != ArPlaybackStatus.Finished && Input.GetKeyDown(KeyCode.R))
                    {
                    ...
                    if (File.Exists(m_Mp4Path) && Input.GetKeyDown(KeyCode.P))
                    {
                ...
                m_Log.Log(m_PlaybackStatusMessage);
                m_Log.Log(m_RecordingStatusMessage);
            }
            else
            {
                m_Log.Log("No " + nameof(ARCoreSessionSubsystem) + " available. Cannot perform session recording.");
            }
#else
            m_Log.Log("ARCore session recording is only supported on Android.");
#endif
        }
    }
}